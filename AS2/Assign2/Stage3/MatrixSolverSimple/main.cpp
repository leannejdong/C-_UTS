...
again:
	...
	char ans2;
	cout<<"do u want to check result multiplication? Press y to check and press any char to skip this"<<endl;
	cin>>ans2;
	if (ans2=='y') {
		int i,j,k;
	...
	matrix trans1;
	trans1=transpose(m);
	cout<<"The transpose of matrix"<<m<<" is "<<printmatrix(trans1);
...
void getvector(matrix p,matrix&m)
{
	...
	cout<<"The rhs of matrix "<<p.matname<<" is "<<m.colvector[0]<<" "<<m.colvector[1]<<" "<<m.colvector[2]<<endl;
}
...
matrix minor(matrix &m, int i,int j)
{
	...
	int row=0,col=0;
	for(int p=0;p<m.n;p++) // outer loop traverses through rows as usual
	{
		for(int q=0;q<m.n;q++) // inner loop traverses through columns as usual
		{
...
{
	return mm.mat[0][0]*mm.mat[1][1]-mm.mat[0][1]*mm.mat[1][0];
}
...
double getdeterminant(matrix m)
{
	...
	if(m.n==1)
		...
		for (int scan =0; scan<m.n; scan++){
			det=det+m.mat[0][scan]*int(pow(-1.0,(1+scan)))*getdeterminant(minor(m,1,scan));
		}
	return det;
}
matrix adjoint(matrix m)
