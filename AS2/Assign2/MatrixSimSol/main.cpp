...
//		getch();
	...
	cout<<endl<<endl;
	char ans2;
	cout<<"Do you want to display the result from matrix multiplication? If yes, Press y "<<endl;
	cin>>ans2;
	if(ans2=='y')
	{
		for (i=1; i<=m.n; i++) {
...
/*matrix mult3by3(matrix &f,matrix &s)
 {
 int i,j,p;
 //double sum;
 double tempresult;
 ...
 tempresult=0;
 for (int p=1;p<=m.n; p++) {
 tempresult+=f.mat[i][p]*s.mat[p][j];
 }
 temp.mat[i][j]=tempresult;
 cout<<temp.mat<<endl;
 }
 }
 return temp;
...
/*void mult3by3(matrix&f,matrix m)
 {
 matrix conc;
 matrix *a;
 for (int i = 0;i <m.n; i++){
 for (int j = 0; j < m.n; j++){
 conc.mat[i][j] =(f.mat[0][j] * m.mat[i][0])+(f.mat[1][j] * m.mat[i][1])+(f.mat[2][j] * m.mat[i][2])+(f.mat[3][j] * m.mat[i][3]);
 }
 }
